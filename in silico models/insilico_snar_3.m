function [STY,E_factor] = insilico_snar_3(Res_time,Temp,C1_inlet_conc,C2_eq,kl_ref_input_1,kl_ref_input_2)
...
Concentration_final = Concentration(Res_time,Temp,C1_inlet_conc,C2_inlet_conc,kl_ref_input_1,kl_ref_input_2);
...
function C_fin = Concentration(Res_time,Temp,C1,C2,kl_ref_input_1,kl_ref_input_2)
...
kl_ref =[57.9,2.70,0.865,1.63];

% levels of the categorical variables
kl_1_options = [49.21,57.9,66.58];
kl_3_options = [0.7353,0.8650,0.9947];

% fixing the levels 
kl_ref(1) = kl_1_options(kl_ref_input_1);
kl_ref(3) = kl_3_options(kl_ref_input_2);

kl_ref = kl_ref.*0.01*(1/0.0166667); % mol-1 dm3 s-1 (to) mol-1 l min-1
